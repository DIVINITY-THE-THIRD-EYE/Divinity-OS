...
void main() {
  ...
  Widget createWidgetUnderTest() {
    return ProviderScope(
      ...
      child: const MaterialApp(home: StudentSupportScreen()),
    );
  }

  testWidgets('displays empty state when no tickets exist', (tester) async {
    when(
      () => mockSupportRepo.fetchMyTickets('student-1'),
    ).thenAnswer((_) async => []);
  ...
  testWidgets(
    'displays support ticket list and status chips when tickets exist',
    (tester) async {
      final ticketsList = [
        SupportTicket(
          id: 'tkt-1',
  ...
  testWidgets(
    'opens creation dialog, validates, and creates a support ticket',
    (tester) async {
      when(
        () => mockSupportRepo.fetchMyTickets('student-1'),
      ).thenAnswer((_) async => []);
      ...
      expect(
        find.text('Subject must be at least 5 characters'),
        findsOneWidget,
      );
      expect(
      ...
      await tester.enterText(
        ...
        'UPI payment transaction failed.',
      );
      await tester.pumpAndSettle();
