          ...
          reportsAsync.maybeWhen(
            data: (data) => PopupMenuButton<String>(
              ...
              itemBuilder: (_) => [
                ...
                const PopupMenuItem(
                  value: 'memberships',
                  child: Text('Export Memberships'),
                ),
                const PopupMenuItem(
                  value: 'students',
                  child: Text('Export Student Demographics'),
                ),
                const PopupMenuItem(
                  value: 'trainers',
                  child: Text('Export Trainer Reports'),
                ),
                const PopupMenuItem(
                  value: 'events',
                  child: Text('Export Event Analytics'),
                ),
              ],
            ),
                ...
                builder: (ctx) {
                  ...
                  bool isPresetSelected(int days) {
                    ...
                    return end.difference(start).inDays == days &&
                        end.difference(today).inDays.abs() <= 1;
                  }
                  ...
                  bool isThisMonthSelected() {
                    ...
                    return start.year == now.year &&
                        ...
                        filters.endDate!.year == now.year &&
                        filters.endDate!.month == now.month;
                  }
...
class _AttendanceTabState extends ConsumerState<_AttendanceTab> {
  ...
  Future<void> _alertStudent(LowAttendanceAlert alert) async {
    ...
    try {
      ...
      await repo.sendNotification(
        ...
        body:
            'Hello ${alert.studentName}, your attendance rate has dropped to ${alert.attendanceRate.toStringAsFixed(0)}%. Consistency is key in yoga and wellness. See you in the next session!',
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Failed to alert: $e')));
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    final avgRate = widget.data.attendance.studentAttendancePercentage.isEmpty
        ? 0.0
        : widget.data.attendance.studentAttendancePercentage.values.reduce(
                (a, b) => a + b,
              ) /
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 8),
                      Text(
                        '${widget.data.attendance.lowAttendanceAlerts.length}',
                        style: tt.headlineSmall?.copyWith(
                          fontWeight: FontWeight.bold,
                        ),
                      ),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                ...widget.data.attendance.perBatchAttendance.entries.map(
                  (e) => Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ListView.separated(
                    ...
                    itemBuilder: (ctx, idx) {
                      final alert =
                          widget.data.attendance.lowAttendanceAlerts[idx];
                      return ListTile(
                        ...
                        subtitle: Text(
                          'Rate: ${alert.attendanceRate.toStringAsFixed(1)}% (${alert.attendedClasses}/${alert.totalClasses} classes)',
                        ),
...
class _FinancialsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final tt = Theme.of(context).textTheme;
    final totalRev = data.revenue.dailyRevenue.fold<double>(
      0.0,
      (s, r) => s + r.value,
    );
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 8),
                      Text(
                        '₹${data.revenue.outstandingFees.toStringAsFixed(0)}',
                        style: tt.headlineSmall?.copyWith(
                          fontWeight: FontWeight.bold,
                          color: AppColors.error,
                        ),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                ...data.revenue.revenueByPlan.entries.map(
                  (e) => Padding(
                    padding: const EdgeInsets.symmetric(vertical: 6),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      ...
      children: [
        GridView.count(
          ...
          children: [
            _MiniCard(
              label: 'Active',
              value: '${data.membership.activeMemberships}',
              color: AppColors.success,
            ),
            _MiniCard(
              label: 'Expiring',
              value: '${data.membership.expiringMemberships}',
              color: AppColors.warning,
            ),
            _MiniCard(
              label: 'Expired',
              value: '${data.membership.expiredMemberships}',
              color: AppColors.error,
            ),
          ],
        ),
                            ...
                            lineBarsData: [
                              LineChartBarData(
                                spots: List.generate(
                                  ...
                                  (i) => FlSpot(
                                    i.toDouble(),
                                    data.membership.membershipGrowth[i].value,
                                  ),
                                ),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                ...data.students.genderDistribution.entries.map(
                  (e) => Padding(
                ...
                Row(
                  ...
                  children: [
                    ...
                    _TrainerStatMini(
                      ...
                      value: '${t.classesConducted}',
                    ),
                    _TrainerStatMini(
                      label: 'Attendance Handled',
                      value: '${t.attendanceHandled}',
                    ),
                  ],
                ),
              ],
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (ctx, idx) {
        ...
        final statusLabel = e.isCancelled
            ? 'Cancelled'
            : (e.isFull ? 'Full' : 'Open');
        final statusColor = e.isCancelled
            ? AppColors.error
            : (e.isFull ? AppColors.warning : AppColors.success);
