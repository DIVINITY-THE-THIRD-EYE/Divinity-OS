...
class ReportsExportUtils {
  ...
  static Future<void> exportAttendance(
    ...
    buf.writeln(
      'Student,Batch,Attendance Rate %,Total Classes,Attended Classes',
    );
  ...
  static Future<void> exportMemberships(
    BuildContext context,
    ReportsData data,
  ) async {
    final buf = StringBuffer();
  ...
  static Future<void> exportStudents(
    ...
    buf.writeln(
      'Male Students,${data.students.genderDistribution['MALE'] ?? 0}',
    );
    buf.writeln(
      'Female Students,${data.students.genderDistribution['FEMALE'] ?? 0}',
    );
    buf.writeln(
      'Other Students,${data.students.genderDistribution['OTHER'] ?? 0}',
    );
    await _shareCsv(context, buf.toString(), 'student_demographics');
  }

  static Future<void> exportTrainers(
    BuildContext context,
    ReportsData data,
  ) async {
    final buf = StringBuffer();
    buf.writeln(
      'Trainer,Active Batches,Student Count,Classes Conducted,Attendance Handled',
    );
    for (final t in data.trainers) {
      final name = _cleanCsv(t.trainerName);
      buf.writeln(
        '$name,${t.activeBatchesCount},${t.studentCount},${t.classesConducted},${t.attendanceHandled}',
      );
    }
  ...
  static Future<void> exportEvents(
    ...
    for (final e in data.events) {
      ...
      buf.writeln(
        '$title,$date,${e.registrationsCount},${e.attendanceCount},$status',
      );
    }
